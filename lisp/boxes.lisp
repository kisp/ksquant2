...
(defun report-bug (code sf-path enp-path err-path)
  (macrolet ((form (form)
    ...
    (with-open-file (out "/tmp/report" :direction :output
      ...
      (format out "lw: ~A ~A~%" (lisp-implementation-type) (lisp-implementation-version))
      (format out "ksquant version: ~A~%" (asdf:component-version (asdf:find-system :ksquant)))
      (format out "ksquant2 version: ~A~%" (asdf:component-version (asdf:find-system :ksquant2)))
  ...
  (let ((path (capi:prompt-for-file "Where to save report?"
				    :operation :save
				    :filters nil
				    :pathname (merge-pathnames (make-pathname :name (format nil "ksquant2-report-~A" (format-time-stamp))
									      :type "tgz"
